...
$hostel = $_POST['hostel'];
  ...
  if(empty($name) || empty($cno) || empty($p) || strlen($cno)!=10)
	...
	$sql = "INSERT INTO info_visitor(Name, Contact, Purpose, meetingTo, day, 
				 ...
				 Comment,registeredBy,imagePath,studentName, 
				 courseYear, Hostel) VALUES ('$name','$cno','$p',
				 '$meet', '$day', '$month', '$year', '$date',
				 '$timein','$rid','ONLINE', '$comment', 
				 '$user_', '$dir', '$studentName', 
				 '$Cyear', '$hostel')";
...
<div class="form-group">
 <label for ='#'> Number of People :</label>
  <input autocomplete="off" class="form-control" type="number" required id = "peopleCount" name = "NofPeople"
	 placeholder="Number of people ?" oninvalid="this.setCustomValidity(this.willValidate?'':'Please Enter how many people are there')"  min="1" onblur="isEmpty('peopleCount')" 
         data-toggle="popover" title="Popover Header" data-content="Some content inside the popover" data-trigger = "onfocus"/>
</div>
</div>
</div>
...
<div class="col-sm-5">
 <label style="padding-top:25px;">Parents ? : &nbsp;&nbsp;<span>
	 <input autocomplete="off" id = "parents" name ="parents" type="checkbox"data-toggle="collapse" 
		data-target="#clgStudent"/></span>
 </label>
<script>
     ...
     <div class="form-group">
      <label for = "nameStudent">Name of Student :</label>
       <input type="text" class="form-control" id="studentName" name="studentName" 
	      placeholder="Student's Name" oninvalid="this.setCustomValidity(this.willValidate?'':'Enter Name !)" 
	      maxlength="30" />
     </div>
   </div>
  <div class= "col-sm-6">
    <div class="form-group">
      <label for = "nameStudent">Course Year :</label>
    	<input type="number" class="form-control" name="Cyear" id = "Cyear" 
	       placeholder="1/2/3/4" oninvalid="this.setCustomValidity(this.willValidate?'':'Choose Course year from 1 to 4)" 
	       maxlength="1" value = 0 />
    </div>
   </div>
 </div>
